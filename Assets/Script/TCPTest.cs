...
public class TCPTest:MonoBehaviour
{
    ...
    Double TriggerredTime, ReactTime, Diff;
    ...
    void SocketReceive()
    {
        ...
        while(true)
        {
            ...
            while(IsReacted == false);
            IsReacted = false;
            TriggerredTime = Double.Parse(recvStr);
            Diff = ReactTime - TriggerredTime;
            print(ReactTime);
            print(TriggerredTime);
            Debug.Log(string.Format("Diff = {0}", Diff));
        }
    }
    ...
    void Update()
    {
        if(Input.GetKeyDown(KeyCode.Space)){
            ReactTime = (Double)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalMilliseconds;
            IsReacted = true;
        }
    }
